...
//    }
    ...
    protected HttpServer createServer(int port) {
        return httpServer(port, new LogMonitor(new DefaultLogFormatter(), new StdLogWriter()));
    }

    @ParameterizedTest(name = "{0}")
    @EnumSource(value = Method.class, names = {"HEAD", "CONNECT"}, mode = Mode.EXCLUDE)
    public void should_proxy_with_request_method(Method method) throws Exception {
        server.request(and(by(uri("/target")), by(method(method.name())))).response("proxy");
        server.request(by(uri("/proxy"))).response(proxy(remoteUrl("/target")));

        Request request = Request.create(method, new URI(remoteUrl("/proxy")));
        running(server, () -> {
            assertThat(helper.executeAsString(request), is("proxy"));
        });
    }
